...
let GetProductMatrixFromQuery = function(ncUtil, channelProfile, flowContext, payload, callback) {
    ...
    async function buildMatrixChildLookupRequest(obj) {
      logInfo(`Matrix Product internalId: ${obj.record.$attributes.internalId}. Lookup up child products...`);
      let searchPayload = {
        "searchRecord": {
          "$attributes": {
            "$xsiType": {
              ...
              "searchValue": {
                "$attributes":{
                  "$xsiType":{
                   ...
                   "internalId": obj.record.$attributes.internalId,
                   "type": "inventoryItem"
                }
              }
            },
    ...
    async function processMatrixChildProducts(lookupResult) {
      logInfo("Processing Matrix Child Products...");
      if (lookupResult.searchResult) {
        if (lookupResult.searchResult.status.$attributes.isSuccess === "true") {
          let records = [];
          if (lookupResult.searchResult.recordList) {
            if (nc.isObject(lookupResult.searchResult.recordList.record)) {
              records.push({ "record": lookupResult.searchResult.recordList.record });
            } else {
              lookupResult.searchResult.recordList.record.forEach(function(record) {
                records.push({ "record": record });
              });
            }
          } else {
            throw new Error("Matrix Product does not have any child items.");
          }
          return records;
        } else {
          if (lookupResult.searchResult.status.statusDetail) {
            out.ncStatusCode = 400;
            out.payload.error = lookupResult.searchResult.status.statusDetail;
            throw new Error(JSON.stringify(lookupResult.searchResult.status.statusDetail));
          } else {
            out.ncStatusCode = 400;
            out.payload.error = lookupResult;
            throw new Error(JSON.stringify(lookupResult.searchResult.status.statusDetail));
          }
        }
      } else {
        out.ncStatusCode = 400;
        out.payload.error = lookupResult;
        throw new Error(lookupResult.searchResult.status.statusDetail);
      }
    }
              ...
              function processResult() {
                return new Promise((pResolve, pReject) => {
                  if (nc.isObject(result.searchResult.recordList.record)) {
                    let records = [];
                    records.push({ record: result.searchResult.recordList.record });

                    buildMatrixChildLookupRequest(result.searchResult.recordList)
                      .then(callNetsuite)
                      .then(processMatrixChildProducts)
                      .then((childProducts) => {
                        docs.push({
                          doc: { "records": records.concat(childProducts) },
                      ...
                      .catch((err) => {
                        logError('Failed to retrieve matrix child products: ' + err);
                        pReject(err);
                      });
                  } else {
                    let promises = [];
                    for (let i = 0, p = Promise.resolve(); i < result.searchResult.recordList.record.length; i++) {
                      ...
                      p = p.then(_ => new Promise((oResolve, oReject) => {
                        buildMatrixChildLookupRequest(product)
                          .then(callNetsuite)
                          .then(processMatrixChildProducts)
                          .then((childProducts) => {
                            docs.push({
