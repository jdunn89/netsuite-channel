...
let GetProductVirtualMatrixFromQuery = function(ncUtil, channelProfile, flowContext, payload, callback) {
    ...
    validateFunction().then(() => {
      if (payload.doc.pagingContext) {
        ...
        createSoapClient()
            .then(async () => {
              // Search Virtual Matrix Parent Products
            ...
            .then(async (result) => {
              ...
              // Search Virtual Matrix Child Products
    ...
    function createRequest(type = "matrix", parentIDs = null) {
        ...
        if (flowContext) {
          ...
          if (flowContext.virtualMatrixParentFlag && flowContext.virtualMatrixParentFlagCriteria && type === "matrix") {
            let obj = {
              "$attributes": {
                "$xsiType": {
          ...
          // Set a filter to include a custom field referencing a parent on the child product - Returns child products where this field is not empty
          if (flowContext.parentIdField && type === "matrixChild") {
